...
function disputeStatusLabel(state: VendorDisputeState, openDisputeCount: number): string {
  if (state === 'OPEN') {
    return `Dispute: Not sorted (${openDisputeCount} open)`;
  }
  if (state === 'SORTED') {
...
export function PayoutManagementView({
  ...
  function openReleaseDialog(row: PayoutDashboardQueueItem) {
    ...
    setReleaseTarget({
      ...
      warning:
        row.eligibility === 'BLOCKED' && row.openDisputeCount > 0
          ? `This vendor has ${row.openDisputeCount} open dispute(s). Record the bank transfer only after you have paid them manually.`
          : !row.hasBankDetails
